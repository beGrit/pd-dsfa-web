dsf.define(function () {
  return {
    ...
    //=======二次开发额外增加的钩子函数 end   ===============
    methods: {
      showLog: function (res) {
        var data = res.data;
        var address = data['dsfa_job_log_findlogsjy.executor_address'];
        if (address.startsWith('127.0.0.1') && address != location.host) {
          address = location.host;
        }
        var defaultRoot = dsf.config.setting_public_web_root_default || '/';
        if (!defaultRoot.startsWith('/')) {
          defaultRoot = '/' + defaultRoot;
        }
        if (!defaultRoot.endsWith('/')) {
          defaultRoot += '/';
        }
        var url = address + defaultRoot + 'job/api/log?path=' + encodeURIComponent(data['dsfa_job_log_findlogsjy.trigger_time'].split(' ')[0] + '/' + data._id + '.log');
        ...
        this.$http.get(url, {})
          .done(({
            ...
            if (success) {
              this.$openDialog({
                ...
                footerAlign: 'center',
                // 
                template: '<pre nowrap>' + data.replace(/\<br\>/g, '')
                  .replace(/xxl\-job/g, 'dsfa-job') + '</pre>',
                // 定义按钮
              });
          ...
          .catch(() => {
            dsf.layer.message('执行器未启动，无法查看日志', false);
          })
          .always(() => {});
      }
    }
