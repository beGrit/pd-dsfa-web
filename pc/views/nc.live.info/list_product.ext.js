dsf.define(function () {
  return {
    ...
    methods: {
      ...
      batchEnabledOn: function () {
        this.batchEnabled("启用", "1")
      },

      batchEnabledOff: function () {
        this.batchEnabled("停用", "0")
      },
      batchEnabled(state_text, state_value) {
        let data = this.$refs['liveList'].multipleSelection;
        let ids = [];
        if (data && data.length > 0) {
          ...
          let params = {
            'liveIds': ids.toString(),
            'state_text': state_text,
            'state_value': state_value
          };
          this.dsf.http.post("nc/live/updatestatus", params)
            .done(res => {
              if (res.success) {
