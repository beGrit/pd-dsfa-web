dsf.define(function () {
  return {
    ...
    computed: {
      zblx: function () {
        if (this.$viewData['nc_live_info.type']) {
          return this.$viewData['nc_live_info.type'].value;
        }
        return ""
      }
    ...
    methods: {
      ...
      backCoursewareAdd: function (res) {
        this.$refs.courseware.value.push({
          "text": res["nc_courses_courseware.title"],
          "value": res["_id"]
        });
      },
      //新增频道回调填充数据
      polyvchannel: function (res) {
        this.$refs.channel.value.push({
          "text": res["nc_live_channel.title"],
          "value": res["_id"]
        });
      }
