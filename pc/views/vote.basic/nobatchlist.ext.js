dsf.define(function () {
  return {
    ...
    methods: {
      //删除
      rowDelFun: function (row) {
        var voteId = row.data["_id"];
        var _this = this;
        _this.loading = true
        var url = "vote/basic/delVote?voteId=" + voteId;
        _this.$http.post(url)
          .done(function (res) {
            if (res.success) {
      ...
      //发布
      rowPublishFun: function (row) {
        var batchId = row.data['vote_datasource_nobatchdata.vote_basic_batch_id'];
        this.changePublishStatus(batchId, "1")
      },
      //取消发布
      rowCancelPublishFun: function (row) {
        var batchId = row.data['vote_datasource_nobatchdata.vote_basic_batch_id'];
        this.changePublishStatus(batchId, "0")
      },
      //修改发布状态
      changePublishStatus(batchId, type) {
        //默认发布url
        var url = "vote/basic/batch/publish";
        //取消发布
        if (type == "0") {
          url = "vote/basic/batch/cancelPublish";
        }
        var _this = this;
        _this.loading = true
        url += "?batchId=" + batchId;
