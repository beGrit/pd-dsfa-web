dsf.define(function () {
  return {
    ...
    postDataBefore: function (next, data, options) {
      console.log(data)
      data.nc_migration_resource.linkurl = nc_migration_resource.r_address + ":" + nc_migration_resource.r_port + "/" + nc_migration_resource.r_name + "?useUnicode=true&characterEncoding=utf-8&useOldAliasMetadataBehavior=true&useSSL=false&serverTimezone=Asia/Shanghai";
      console.log(data)
      next();
    },
    ...
    methods: {
      selectCourseware: function (res) {
        ...
        if (data && data.length > 0) {
          ...
          param.id = args.id;
          this.dsf.http.get('/nc/courseMigration/sycnCourse', param, {
              headers
            })
            .done(res => {
              if (res.success) {
