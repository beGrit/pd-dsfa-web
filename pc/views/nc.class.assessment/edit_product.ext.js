dsf.define(function () {
  return {
    ...
    dataLoaded: function (data, next) {
      ...
      this.showBox(data['nc_class_assessment.condition'])
    ...
    postDataBefore: function (next, data, options) {
      data.unitList = JSON.stringify(this.unitLists);
    ...
    //=======二次开发额外增加的钩子函数 end   ===============
    methods: {
      eletivesChange(lists) {
        this.unitLists = lists; // 获取的数组
      },
      change(event) {
      ...
      showBox(args) {
        console.log(787878, args)

        if (args.length) { // 初始化为选择了多项 值才为数组，否则为对象
          let value = args[0].value
          if (args.length == 1) {
            if (value == '1') { // 完成学时
              document.querySelectorAll(".ds-section")[2].style.display = 'block';
              document.querySelectorAll(".ds-section")[3].style.display = 'none';
            }
            if (value == '2') { // 完成考试
              document.querySelectorAll(".ds-section")[2].style.display = 'none';
              document.querySelectorAll(".ds-section")[3].style.display = 'block';
            }
          } else {
            console.log('全选')
            document.querySelectorAll(".ds-section")[2].style.display = 'block';
        ...
        } else if (Object.getOwnPropertyNames(args)
          .length) { //初始化只选择了一项时，值为对象时判断
          if (args.value == '1') { // 完成学时
            document.querySelectorAll(".ds-section")[2].style.display = 'block';
            document.querySelectorAll(".ds-section")[3].style.display = 'none';
          }
          if (args.value == '2') { // 完成考试
            document.querySelectorAll(".ds-section")[2].style.display = 'none';
            document.querySelectorAll(".ds-section")[3].style.display = 'block';
          }
