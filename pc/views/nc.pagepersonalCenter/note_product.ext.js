dsf.define(function () {
  return {
    ...
    data: function () {
      return {
        url: '/meta/list/data?pageNum=1&pageSize=15&query=%7B%22searchValue%22%3A%222%22%2C%22ds_create_time%22%3A%5B%5D%7D&order=%5B%5D&filter=%5B%5D&namespace=nc.courses.page&pageName=minenote'
      }
    },
    computed: {},
    created: function () {},
    ...
    methods: {
      handleSearch: function (val) {
        let query = ('{"searchValue":"' + val.args.txt + '","ds_create_time":[]}')
        if (val.args.date[0]) {
          query = ('{"searchValue":"' + val.args.txt + '","ds_create_time":["' + val.args.date[0] + '","' + val.args.date[1] + '"]}')
        }
        this.url = 'meta/list/data?pageNum=1&pageSize=15&query=' + encodeURIComponent(query) + '&order=%5B%5D&filter=%5B%5D&namespace=nc.courses.page&pageName=minenote'
      }
    }
  }
});