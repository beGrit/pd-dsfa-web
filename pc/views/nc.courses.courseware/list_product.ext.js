  ...
  return {
    ...
    methods: {
      ...
      batchStateUp: function () {
        this.batchState('/nc/courseware/list/state/batch/up')
      },
      //课件批量停用
      batchStateDown: function () {
        this.batchState('/nc/courseware/list/state/batch/down')
      },
      batchState(url) {
        let data = this.$refs['coursewarelist'].multipleSelection;
        let ids = [];
      ...
      refreshSliceList: function () {
        ...
        if (data && data.length > 0) {
          ...
          this.refreshSlice(ids);
        } else {
          this.dsf.layer.message('请选择至少一条数据', false)
        }
      },
      refreshSlice: function (ids) {
        let params = {
          'ids': ids
        };
        this.dsf.http.post("/nc/courseware/list/refreshIsSlice", params)
          .done(res => {
            if (res.success) {
              var timer = setTimeout(() => {
                this.reloadData();
