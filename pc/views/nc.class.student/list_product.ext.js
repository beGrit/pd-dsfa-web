dsf.define(function () {
  return {
    ...
    methods: {
      ...
      selectStudent: function (res) {
        var data = res.args;
        let param = [];
        if (data && data.length > 0) {
          let classId = this.dsf.url.queryString('id');
          let groupId = res.target.queryString.pid;
          _.forEach(data, function (d) {
            let info = {
              userId: d['_id'],
              userName: d['nc_class_data_studentSelectData.name'],
              deptName: d['nc_class_data_studentSelectData.deptname'],
              phone: d['nc_class_data_studentSelectData.phone'],
              groupId: groupId,
              classId: classId
            }
            param.push(info)
          ...
          this.dsf.http.post("/nc/class/student/add", JSON.stringify(param), {
              headers
            })
            .done(res => {
              if (res.success) {
