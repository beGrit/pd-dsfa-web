    ...
    methods: {
        ...
        if (data) {
          switch (data['dsfa_rm.type'].value) {
            ...
            case "1":
              ...
              var pname = arr.pop();
              var m = arr.join(".");
              //如果是原型设计目录则进入原型设计状态的设计器
              var layoutType = "page";
              if (b == 'prototype') {
                layoutType = "prototype";
              }
              this.$openWindow("./designer.html#/pc/" + layoutType + "?B=" + b + "&M=" + m + "&pname=" + pname + "&title=" + data["dsfa_rm.name"]);
              break;
            case "2":
            ...
            case "13":
              ...
              this.$openWindow("./designer.html#/mobile/" + layoutType + "?B=" + b + "&M=" + m + "&pname=" + pname + "&title=" + data["dsfa_rm.name"]);
              break;
              //移动端页面模板
            case "14":
              var path = data["dsfa_rm.path"];
              ...
              //移动端app管理  
            case "16":
              this.$openWindow("./designer.html#/mobilesetting?id=" + data['_id']);
              break;
          }
        }
      },
      ...
      openview: function (evt) {
        ...
        if (data) {
          ...
          var client = "pc";
          var htmlPage = "page"
          if (data['dsfa_rm.type'].value == "13" || data['dsfa_rm.type'].value == "14") {
            client = "mobile";
            htmlPage = 'mobile-index'
          }
          this.$openWindow("./" + htmlPage + ".html#/" + client + "/" + b + "/" + m + "/" + pname);
        }
      }
    }
  }
