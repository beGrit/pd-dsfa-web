dsf.define(function () {
  return {
    ...
    dataLoaded: function (data, next) {
      if (data["nc_learn_setting.is_auto_process"] && data["nc_learn_setting.is_auto_process"].value && data["nc_learn_setting.is_auto_process"].value == "1") {
        this.process_rate_isReadOnly = false;
      }
      debugger;
      next();
    },
    ...
    methods: {
      //自定义课件进度
      isAutoFinishProcessChange(v) {
        this.process_rate_isReadOnly = v.args.value == "1" ? false : true;
      }
    }
  }
});