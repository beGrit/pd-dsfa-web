dsf.define(function () {
  return {
    ...
    computed: {
      setCoverImg(item) {
        return item['ipb_album_subData.img'] ? JSON.parse(item['ipb_album_subData.img'])[0].relativePath : '';
      }
    },
    created: function () {
      console.log(this)
    ...
    methods: {
      ...
      toDetail(item, e) {
        ...
        if ($item.hasClass('checkItem')) {
          $item.removeClass('checkItem');
          this.deleteIds = this.remove(item._id);
        } else {
      ...
      deleteItems(arg) {
        ...
        if (this.deleteIds.length == 0) return this.dsf.layer.message('请选择要删除的照片', false);
        var params = {
          // ids: JSON.stringify(this.deleteIds)
          ids: this.deleteIds.join(',')
        };
        this.dsf.http.post('alibum/delByIds', params)
          .done(res => {
            if (res.success) {
            ...
            _this.dsf.layer.message(res.message, res.success);
          })
          .error(err => {
            console.log(err)
            _this.dsf.layer.message('删除异常', false)
          })
          .always((res) => {})
      }
    }
