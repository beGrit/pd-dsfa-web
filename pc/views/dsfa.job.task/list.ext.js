dsf.define(function () {
  return {
    ...
    //=======二次开发额外增加的钩子函数 end   ===============
    methods: {
      triggerChange: function (scope) {
        var switchVal = !~~scope.row['dsfa_job_task.trigger_status'].value;
        this.$http.post("job/task/switch", {
            id: scope.row._id,
            value: switchVal
          })
          .done(({
            ...
            if (success) {
              scope.row['dsfa_job_task.trigger_status'] = {
                value: switchVal ? 1 : 0
              }
              dsf.layer.message(switchVal ? '已启动' : '已关闭', true);
            } else {
              dsf.layer.message('请求异常', false);
            }
          })
          .catch(() => {
            dsf.layer.message('请求异常，可能是未添加定时调度依赖“dsfa-platform-starter-job”', false);
          })
          .always(() => {});
      }
    }
