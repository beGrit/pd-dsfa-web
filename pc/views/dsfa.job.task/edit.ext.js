dsf.define(function () {
  return {
    ...
    data: function () {
      return {
        validateRules: {
          jobMark: function (v, vm) {
            return vm['dsfa_job_task.glue_type'].value == 'BEAN' && _.isEmpty(vm['dsfa_job_task.job_mark']) ? {
              message: '请输入@JobHandler注解的value值'
            } : null
          },
          scheduleType: function (v, vm) {
            return vm['dsfa_job_task.schedule_type'].value != 'NONE' && _.isEmpty(vm['dsfa_job_task.schedule_conf']) ? {
              message: '请输入调度规则'
            } : null
          },
        }
    ...
    saveBefore: function (next, options) {
      // if (!~~this.$refs.trigger_status.value.value) {
      this.$set(this.$viewData, 'dsfa_job_task.trigger_last_time', 0)
      this.$set(this.$viewData, 'dsfa_job_task.trigger_next_time', 0)
      // }
      next();
    },
