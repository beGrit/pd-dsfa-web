  ...
  $.fn.extend({
    circleProgress: function(options) {
      ...
      function progress() {
        ...
        var ang = percent == 0 ? 0.1 : Math.round(((360 * percent) / 100) % 360);
        // 进度条
        ctx.beginPath();
        ctx.lineWidth = opts.lineWidth;
        ctx.strokeStyle = opts.innerColor;
        ctx.arc(canvasWth / 2, canvasWth / 2, opts.radius, 2 * Math.PI * (ang / 360), 2 * Math.PI, true); // true表示顺时针
        ctx.stroke();
        ctx.closePath();
        // 文本
        ctx.save(); // 保存上一个状态
        ...
        var translateX = len == 1 ? parseInt(canvasWth / 3) : len == 2 ? parseInt(canvasWth / 4) : parseInt(canvasWth / 10);
