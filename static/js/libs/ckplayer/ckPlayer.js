...
!(function () {
  ...
  var ckplayer = function (obj) {
    ...
    this.contextMenu = [
      ["ckplayer", "link", "https://www.ckplayer.com", "_blank"],
  ...
  ckplayer.prototype = {
    ...
    interFace: function () {
      ...
      var bBgColor = "#FFFFFF", //按钮元素默认颜色
        bOverColor = this.vars["themeColor"]; //按钮元素鼠标经过时的颜色
      ...
      html += '<div  class="' + nextID + '" data-title="' + thisTemp.language["next"] + '" id="dNext">' + this.newCanvas(nextID, bWidth, bHeight) + "</div>"; //下一集按钮
      html += '<div class="' + dlineID + '-lc"></div>'; //分隔线
      html += '<div class="' + timeTextID + '">' + timeInto + "</div>"; //时间文本

      html += '<div class="' + fullID + '" data-title="' + thisTemp.language["full"] + '">全屏</div>'; //全屏按钮
      html += '<div class="' + escFullID + '" data-title="' + thisTemp.language["escFull"] + '">退出全屏</div>'; //退出全屏按钮
      ...
      html +=
        "<div class='d-autoplay' style='display:" +
        block +
        ";color: #fff;width: 140px;float: right;line-height: 40px;text-align: center;cursor: pointer;font-size:13px;'>自动播放 <span id='J_autoPlay' style='color:#FFF;text-align: left;display:inline-block;width: 42px;height: 18px;background:#d40000;border-radius: 15px;line-height:18px;position:relative;top:5px;'><i style='display: inline-block;width: 90%;position:relative;float:left;text-indent:2px;font-style:normal;' id='J_auto_on'>ON</i><i style='position: absolute;top:2.5px;right:4px;width:12px;height:12px;border-radius:50%;background:#FFF;font-style:normal;' id='J_auto_btn'></i><i id='J_auto_off' style='display: inline-block;width: 0%;float:right;text-align:right;margin-right:2px;font-style:normal;'></i></span></div>"; //自动播放
      html += '<div class="' + dlineID + '-re"></div>'; //分隔线

      this.getByElement(controlBarID).innerHTML = html;
      //构建进度条内容
      if (!this.vars.hideProgress) {
      ...
      if (!this.vars.hideProgress) {
        ...
        this.css(timeProgressID, "backgroundColor", "#2B95FF");
        this.css(timeBWID, {
          width: "10px",
          height: "10px",
          overflow: "hidden",
      ...
      this.css([dlineID + "-la", dlineID + "-lb", dlineID + "-lc", dlineID + "-ra", dlineID + "-rb", dlineID + "-rc", dlineID + "-rd", dlineID + "-re"], {
        ...
        height: bHeight + "px",
        overflow: "hidden",
        borderLeft: "0 solid #303030",
        borderRight: "0 solid #151515",
      ...
      var cNextFillRect = function () {
        ...
        // thisTemp.canvasFill(cNext, [[8, 10], [16, 18], [8, 28]]);
        thisTemp.canvasFillRect(cNext, [[25, 10, 4, 18]]);
      };
      ...
      var timeBOOver = function () {
        thisTemp.css(timeBOID, "backgroundColor", "#3367A4");
        thisTemp.css(timeBWID, "backgroundColor", "#2B95FF");
      };
    ...
    addButtonEvent: function () {
      ...
      var pauseClick = function (event) {
        //直播中不能点击
        // console.log(thisTemp.vars);
        if (thisTemp.vars.islive && !thisTemp.vars.ckplayerPause) {
          return;
        }
        thisTemp.videoPause();
    ...
    checkSlideLeft: function (newX, sliderLeft, refer) {
      ...
      //直播时不允许拖动
      if (this.vars.islive) {
        timeSA = this.vars.timeScheduleAdjust;
      }
      switch (timeSA) {
        case 0:
          return false;
    ...
    keypress: function () {
      ...
      var keyDown = function (eve) {
        ...
        switch (keycode) {
          case 32:
            //不允许暂停
            if (thisTemp.vars.keyDown && !thisTemp.vars.keyDown.space) {
              break;
            } else {
              thisTemp.playOrPause();
              break;
          ...
          case 37:
            //不允许后腿
            if (thisTemp.vars.keyDown && !thisTemp.vars.keyDown.left) {
              break;
            } else {
              thisTemp.fastBack();
              break;
    ...
    embedSWF: function () {
      ...
      var flashplayerUrl = "https://www.flash.cn";
      var html = "",
      ...
      html +=
        '<object pluginspage="' +
        flashplayerUrl +
        '" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"  codebase="https://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=11,3,0,0" width="100%" height="100%" ' +
        id +
    ...
    changeAutoPlay: function () {
      var init = true;
      var color = this.vars["themeColor"];
      var isAuto = localStorage.getItem("isAuto") || 0;
      var dAutoPlay = document.getElementById("J_autoPlay");
      var on = document.getElementById("J_auto_on");
      var btn = document.getElementById("J_auto_btn");
      var off = document.getElementById("J_auto_off");
      var offSty = "position:absolute;left:4px;top:2.5px;width:12px;height:12px;border-radius:50%;background:#fff;font-style: normal;";
      var onSty = "position:absolute;right:4px;top:2.5px;width:12px;height:12px;border-radius:50%;background:#fff;font-style: normal;";
      if (init) {
        init = false;
        if (isAuto == 0) {
          on.innerHTML = "";
          btn.setAttribute("style", offSty);
          off.innerHTML = "OFF";
          on.style.width = "0%";
          off.style.width = "90%";
          dAutoPlay.style.background = "#ccc";
          dAutoPlay.style.fontSize = "12px";
          isAuto = 0;
        } else {
          on.innerHTML = "ON";
          btn.setAttribute("style", onSty);
          off.innerHTML = "";
          on.style.width = "90%";
          off.style.width = "0%";
          dAutoPlay.style.background = color;
          isAuto = 1;
        }
      }
      dAutoPlay.addEventListener("click", function () {
        if (isAuto == 1) {
          on.innerHTML = "";
          btn.setAttribute("style", offSty);
          off.innerHTML = "OFF";
          ...
          dAutoPlay.style.background = "#ccc";
          dAutoPlay.style.fontSize = "12px";
          dAutoPlay.style.fontStyle = "normal";
          isAuto = 0;
        } else {
          on.innerHTML = "ON";
          btn.setAttribute("style", onSty);
    ...
    isMsie: function () {
      ...
      if (
        ...
        (trim_Version == "MSIE6.0" || trim_Version == "MSIE7.0" || trim_Version == "MSIE8.0" || trim_Version == "MSIE9.0" || trim_Version == "MSIE10.0")
      ) {
        return false;
      }
      return true;
