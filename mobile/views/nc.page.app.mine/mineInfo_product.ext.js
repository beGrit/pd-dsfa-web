    ...
    methods: {
      handleClickLoginOut: function () {
        ...
        this.dsf.http
          ...
          .done(res => {
            if (res.success) {
              this.dsf.layer.mobile.message(res.message, true);
              setTimeout(() => {
                this.$router.push('/mobile/login')
              }, 500)
            } else {
              this.dsf.layer.mobile.message(res.message || "退出失败", false);
            }
          })
          .error(response => {
            this.dsf.layer.mobile.message(response.message || "退出失败", false);
          });
      }
    }
  }
