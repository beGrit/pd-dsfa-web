  ...
  return {
    ...
    methods: {
      getData: function (callback) {
        let url = "/mobile/evaluate/detail/" + this.queryString.id;
        var _this = this;
        this.$http.get(url)
          .then(function (res) {
            if (res.data.success) {
              if (res.data.data && res.data.data.score) {
                ...
                _this.$set(_this.listData, 'hideBtn', true);
                // _this.listData.hideBtn = true;
              } else {
                // _this.listData.hideBtn = false;
                _this.$set(_this.listData, 'hideBtn', false);
              }
            }
          })
      ...
      submit: function () {
        ...
        var url = '/meta/persistData'

        var baseUrl = $$webRoot.default
        var params = {
          namespace: 'idj.evaluation',
          pageName: 'edit',
          attach: JSON.stringify({}),
          data: JSON.stringify({
            'idj_evaluation.yw_id': this.queryString.id,
            'idj_evaluation.score': this.listData.score,
            'idj_evaluation.content': this.listData.content
          })
        }
        this.clickFlag = true
        var _self = this
        this.$http.post(url, params, null, baseUrl)
          .then(function (
            res
          ) {
            var data = res.data
