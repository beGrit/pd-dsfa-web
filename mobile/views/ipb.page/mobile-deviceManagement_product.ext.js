  ...
  return {
    ...
    methods: {
      ...
      removeDevice: function () {
        var that = this;
        this.$http.post("/mobile/device/remove", {
            id: this.$viewData['_id']
          })
          .then(function (res) {
            dsf.layer.message(res.data.message);
            setTimeout(function () {
              that.$router.replace("/mobile/ipb/page/mobile-unitManagement");
            }, 2000)
          });
      },
      // 启用设备
      ...
      setDeviceStatus: function (status) {
        ...
        this.$http.post("/mobile/device/changeStatus", param)
          .then(function (res) {
            dsf.layer.message(res.data.message);
            that.$viewData["ipb_page_deviceDetail.state_value"] = status;
          });
      }
    }
  }
