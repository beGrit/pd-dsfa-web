dsf.define(function () {
  return {
    ...
    ready: function () {
      var that = this;
      that.$http.get("/mobile/unit/device", {})
        .then(function (res) {
          that.orgName = res.data.data.unitName;
        })
    ...
    methods: {
      ...
      createInviteCode: function () {
        var that = this;
        this.$http.get("/dsfa/register/inviteCode", {})
          .then(function (res) {
            that.invitationCode = res.data.data.split("");
          });
      },
      ...
      createQrCode: function () {
        ...
        var param = {
          code: "inviteCode",
          businessId: that.invitationCode.join(""),
          size: 100
        };
        this.$http.get("/dsfa/nc/ncqrcode/buildQrCodeQd", param)
          .then(function (res) {
            if (res.data && res.data.success) {
              that.qrCode = "data:image/jpg;base64," + res.data.data;
              vant.ImagePreview([that.qrCode]);
            } else {
              dsf.layer.message(ds.message, false);
            }
          });
      },
    }
