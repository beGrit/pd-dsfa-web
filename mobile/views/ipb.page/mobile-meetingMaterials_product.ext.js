  ...
  return {
    ...
    methods: {
      ...
      handleShare() {
        ...
        // });
        window.shareData = {
          imgUrl: "https://wyximglive.wyxer.com/upfile2020/1586253697000.jpg",
          link: "https://y.wyxer.com/wxwall/bigscreen/pclive/index.php?aid=441",
          title: "北京越野团购会直播.（电脑端登陆入口）",
          desc: "直播：2020-04-12  15点",
          success: function () {
            // 设置成功
            dsf.layer.message("分享成功");
          }
        };
      },
      ...
      // 文件预览
      previewFile(row) {
        var file = row["adj_meeting_meetingFileForApp.sp"] || row["adj_meeting_meetingFileForApp.wg"] || row["adj_meeting_meetingFileForApp.yp"];
        var fileId = JSON.parse(file)[0].id;
        location.href = dsf.url.getWebPath(`/dsfa/preview/file?fileId=${fileId}`);
      }
    }
  }
});