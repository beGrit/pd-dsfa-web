dsf.define(function () {
  return {
    ...
    saveAfter: function (next, result, options) {
      sessionStorage.removeItem('needPromise')
      this.$dialog.close()
      next();
    },
    ...
    methods: {
      getContent: function () {
        var _this = this
        _this.$http.get('teas/mobile/studentClassNotice/content', {})
          .done(function (res) {
            if (res.success) {
              _this.html = res.data.content
              _this.setReadTime(res.data.limittime)
            } else {
      ...
      setReadTime(time) {
        ...
        var _this = this
        _this.$refs.buttonbar4.buttons[0].text = '我承诺（' + time + ' s）'
        var timer = setInterval(function () {
          time--
          if (time > 0) {
            _this.$refs.buttonbar4.buttons[0].text = '我承诺（' + time + ' s）'
          } else {
            _this.$refs.buttonbar4.buttons[0].text = '我承诺'
